...
internal static class SerranoTests
{
    public static void Run(Action<string, Action> check)
    {
        ...
        var stages = Enumerable.Range(1, 11).ToDictionary(id => id,
            id => JObject.Parse(File.ReadAllText(Path.Combine(assets, $"shapes/block/plant/crop/serrano/stage{id}.json"))));
        var item = JObject.Parse(File.ReadAllText(Path.Combine(assets, "shapes/item/food/vegetable/serrano.json")));
        ...
        check("all serrano shapes load with valid rooted geometry and runtime textures", () => {
            foreach (var json in stages.Values.Append(item))
            {
                ...
                foreach (var element in json["elements"].Skip(1))
                {
                    ...
                    foreach (var face in ((JObject)element["faces"]).Properties().Select(p => p.Value))
                    {
                        ...
                        var textureMap = ReferenceEquals(json, item) ? food["texturesByType"]["*-serrano"] : plantDefinition["textures"];
                        string runtimePath = ((string)textureMap[key]?["base"])?.Replace("{type}", "serrano");
        ...
        check("serrano maturity harvest regrowth and dormancy preserve the bush skeleton", () => {
        ...
        check("serrano fruit grows to green harvest size without moving its attachment", () => {
            foreach (int id in new[] { 6, 7, 8, 11 })
                Require(FruitTextures(stages[id]).SetEquals(new[] { "#pepper" }), "Serranos are harvested green");
            foreach (int id in new[] { 6, 7, 8, 11 })
            {
                ...
                foreach (var calyx in current.Elements.Where(e => e.Name.EndsWith("-calyx-collar")))
                {
                    ...
                    if (id != 8) Require(calyx.To[0] - calyx.From[0] < ripe.To[0] - ripe.From[0], "Immature fruit must be smaller");
        ...
        check("serrano leaves and fruit stalks stay attached to their supporting branches", () => {
        ...
        check("serrano wind pins leaf bases and keeps all stems flowers and fruit rigid", () => {
        ...
        check("ripe serrano harvest drops serranos and retains the mature plant", () => {
            var fixture = new Fixture();
            fixture.Block.Code = new AssetLocation("peppermod", "crop-serrano-8");
            fixture.Harvested.Code = new AssetLocation("peppermod", "crop-serrano-10");
            fixture.Start(); fixture.Step(1.5f); fixture.Stop(1.5f);
            Require(fixture.CurrentBlock.Code.Path == "crop-serrano-10", "Serrano did not enter harvested state");
            int quantity = fixture.Drops.Sum(drop => drop.Stack.StackSize);
            Require(quantity >= 16 && quantity <= 24, "Incorrect serrano harvest size");
            Require(fixture.Drops.All(drop => drop.Stack.Collectible.Code.Path == "vegetable-serrano"
                && drop.Velocity.X == 0 && drop.Velocity.Z == 0 && drop.Velocity.Y < 0), "Incorrect serrano drops");
        });
        check("serrano bites add forty spice and twenty satiety with the normal heat cap", () => {
            Require(food["nutritionPropsByType"]["*-serrano"]["satiety"].Value<int>() == 20, "Unexpected satiety");
            float heat = food["attributesByType"]["*-serrano"]["peppermodSpice"].Value<float>();
            Require(heat == 40, "Unexpected serrano heat");
            var fixture = new FoodFixture(count: 3, heat: heat);
            fixture.Food.Code = new AssetLocation("peppermod", "vegetable-serrano");
            fixture.Food.Eat(1, fixture.Slot, fixture.Player);
            Require(fixture.Player.Saturation == 20 && PepperSpiceSystem.ReadState(fixture.Player).Heat == 40
                && PepperSpiceSystem.ReadState(fixture.Player).Level == SpiceLevel.Hot, "First serrano should reach Hot at 40 spice");
            PepperSpiceSystem.TickPlayer(fixture.Player, 2);
            fixture.Food.Eat(1, fixture.Slot, fixture.Player);
            Require(fixture.Player.Saturation == 40 && PepperSpiceSystem.ReadState(fixture.Player).Heat == 80
                && PepperSpiceSystem.ReadState(fixture.Player).Level == SpiceLevel.Extreme, "Second serrano should reach Extreme at 80 spice");
            fixture.Food.Eat(1, fixture.Slot, fixture.Player);
            Require(PepperSpiceSystem.ReadState(fixture.Player).Heat == 100, "Further serranos should respect the heat cap");
        });
        check("serrano seeds use finished artwork and the standard plantable seed class", () => {
            var seeds = JObject.Parse(File.ReadAllText(Path.Combine(assets, "itemtypes/seeds.json")));
            Require((string)seeds["class"] == "ItemPepperSeeds" && seeds["variantgroups"][0]["states"].Values<string>().Contains("serrano"), "Serrano seed variant missing");
            using var bitmap = new Vintagestory.API.Common.BitmapExternal(Path.Combine(assets, "textures/item/seeds/serrano.png"), null);
            Require(bitmap.Width >= 64 && bitmap.Height >= 64 && bitmap.Pixels.Any(p => ((uint)p >> 24) > 0), "Placeholder or blank seed artwork");
        });
    }
